...
from Dataloader import Phoneme
...
## 按类别划分样本,并进行正则化
# 划分样本
low_income_samples = adult[adult["class"] == 0]
high_income_samples = adult[adult["class"] == 1]
...
d = dice_ml.Data(dataframe=train_dataset,
                 continuous_features=["V1","V2","V3","V4","V5"],
                 outcome_name='class')

numerical = ["V1","V2","V3","V4","V5"]
...
dice_exp = k_exp.generate_counterfactuals(low_income_query.drop("class",axis=1),total_CFs=n_cfs*len(low_income_query),desired_class="opposite")
dice_CFs = dice_exp.cf_examples_list[0].final_cfs_df
dice_CFs.insert(5,"class",np.array([1 for i in range(len(dice_CFs))]))
dice_CFs.to_csv("dice_k.csv",index = False)
print("kdtree ending.")

r_exp = dice_ml.Dice(d,m,method="random")

dice_exp = r_exp.generate_counterfactuals(low_income_query.drop("class",axis=1),total_CFs=n_cfs*len(low_income_query),desired_class="opposite")
dice_CFs = dice_exp.cf_examples_list[0].final_cfs_df
# dice_CFs.insert(9,"class",np.array([1 for i in range(len(dice_CFs))]))
dice_CFs.to_csv("dice_r.csv",index = False)
print("random ending.")

g_exp = dice_ml.Dice(d,m,method="genetic")

dice_exp = g_exp.generate_counterfactuals(low_income_query.drop("class",axis=1),total_CFs=n_cfs*len(low_income_query),desired_class="opposite")
dice_CFs = dice_exp.cf_examples_list[0].final_cfs_df
