...
## 按类别划分样本,并进行正则化
# 划分样本
low_income_samples = adult[adult["income"] == 0]
high_income_samples = adult[adult["income"] == 1]

clf1 = Net("/datas/wangm/GLT-fastCFs/Adult/v_model.joblib")
...
dice_exp = k_exp.generate_counterfactuals(low_income_query.drop("income",axis=1),total_CFs=n_cfs*len(low_income_query),desired_class="opposite")
...
dice_exp = r_exp.generate_counterfactuals(low_income_query.drop("income",axis=1),total_CFs=n_cfs*len(low_income_query),desired_class="opposite")
dice_CFs = dice_exp.cf_examples_list[0].final_cfs_df
# dice_CFs.insert(8,"income",np.array([1 for i in range(len(dice_CFs))]))
...
dice_exp = g_exp.generate_counterfactuals(low_income_query.drop("income",axis=1),total_CFs=n_cfs*len(low_income_query),desired_class="opposite")
dice_CFs = dice_exp.cf_examples_list[0].final_cfs_df
# dice_CFs.insert(8,"income",np.array([1 for i in range(len(dice_CFs))]))
